...
async function main() {
  ...
  console.log(
    "Deployed Real Estate Contract at:",
  ...
  for (let i = 0; i < 3; i++) {
    // Approve properties...
    let transaction = await realEstate.connect(seller).approve(add1, i + 1);
    await transaction.wait();
  }
  ...
  transaction = await escrow
    .connect(seller)
    .list(1, add2, tokens(20), tokens(10));
  await transaction.wait();

  transaction = await escrow
    .connect(seller)
    .list(2, add2, tokens(15), tokens(5));
  await transaction.wait();

  transaction = await escrow
    .connect(seller)
    .list(3, add2, tokens(10), tokens(5));
  await transaction.wait();
